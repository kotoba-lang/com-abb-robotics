(ns abb_robotics.main
  "Kotodama WASM entrypoint for the Abb_robotics clean-room actor (L4) — Clojure port.
  ...
  facts (`abb_robotics.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [kotoba.lang.text :as str]))

(def ns-prefix "abb_robotics")
(def tier "L4")
...
(def entity-specs
  [{:entity "Part"        :plural "parts"        :id-prefix "abbrobot_par"
    :fields [:partNumber :name :revision :material]  :required [:partNumber :name]
    :coerce {}                       :refs {}}
   {:entity "BOM"         :plural "boms"         :id-prefix "abbrobot_bom"
    :fields [:parentPartId :childPartId :quantity]   :required [:quantity]
    :coerce {:quantity :int}         :refs {:parentPartId "Part" :childPartId "Part"}}
   {:entity "ChangeOrder" :plural "changeorders" :id-prefix "abbrobot_cha"
    :fields [:partId :number :status]                :required [:number :status]
    :coerce {}                       :refs {:partId "Part"}}
   {:entity "WorkOrder"   :plural "workorders"   :id-prefix "abbrobot_wor"
    :fields [:partId :quantity :status]              :required [:quantity :status]
    :coerce {:quantity :int}         :refs {:partId "Part"}}
   {:entity "Robot"       :plural "robots"       :id-prefix "abbrobot_rob"
    :fields [:model :cellId :status :payloadKg]     :required [:model :status]
    :coerce {:payloadKg :float}      :refs {}}
   {:entity "Program"     :plural "programs"     :id-prefix "abbrobot_pro"
...
(defn as-int-kernel
  ...
  of abb_robotics.main/as-int stays here as the retained oracle."
  [v]
  (if (pos? v) v 0))
...
(defn emit-facts
  "EAVT facts for one record: {\"abb_robotics.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "abb_robotics-compat" :tier tier :entities entities} 200])
...
(defn start! [] :abb_robotics-compat/ready)
